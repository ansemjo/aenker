...
func NewAenker(key *[]byte, chunksize int) (ae *Aenker) {

	aead := aeadCipher{
		New:  chacha20poly1305.New,
		NewX: chacha20poly1305.NewX,
	}
	return &Aenker{aead, key, nil, chunksize}
}
...
func (ae *Aenker) Encrypt(w io.Writer, r io.Reader) (lengthWritten int64, err error) {

	// TODO: add lengthWritten output to sealMEK, otherwise lw only counts chunkstream
	err = ae.sealNewMEK(w)
	if err != nil {
		return
	}

	return ae.encryptChunkStream(w, r)
...
func (ae *Aenker) Decrypt(w io.Writer, r io.Reader) (lengthWritten int64, err error) {
